...
function scoreCommitFreshness(snapshot, now, reasons) {
...
function scoreBinaryRatio(snapshot, reasons) {
...
function scoreConventionalCompliance(snapshot, reasons) {
    if (!snapshot.lastCommit.sha || !snapshot.lastCommit.subject) {
        if (!reasons.includes("no commits"))
            reasons.push("no commits");
        return 0;
    }
